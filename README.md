...
脚本会拉取 `https://github.com/ImageMagick/Windows`，同步 IM7/delegate 源码，并把 headers、import libs、DLL、modules、配置 XML 和许可文件提取到：
...
如果输出文件重名，默认覆盖。批处理会按扫描顺序处理同名输出，最后写入的文件保留；`summary.csv` 的总体积按最终唯一输出统计。
...
- 运行时 DLL、modules 和配置文件在构建后复制到程序输出目录，运行时不依赖 PATH 中的 `magick.exe`
